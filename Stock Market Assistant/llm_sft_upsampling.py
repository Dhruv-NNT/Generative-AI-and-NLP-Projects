...
upsampled_dataset = pd.concat([
    ...
    resample(train_dataset[train_dataset['label'] == 1], n_samples=max_samples, replace=True, random_state=42),  # Neutral class
    resample(train_dataset[train_dataset['label'] == 2], n_samples=max_samples, replace=True, random_state=42)  # Positive class
])

# Shuffle the upsampled dataset
upsampled_dataset = upsampled_dataset.sample(frac=1, random_state=42).reset_index(drop=True)

# Split the upsampled dataset into training and remainder
train_dataset, remainder_dataset = train_test_split(upsampled_dataset, test_size=0.2, random_state=42)
